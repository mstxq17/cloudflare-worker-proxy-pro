...
</style></head><body><section class="hero"><div class="card"><div class="badge">${escapeHtml(runtime.mainDomain)}</div><h1 class="title">TCP proxy at the edge.</h1><p class="lead">将每个子域名映射到独立 HTTP/HTTPS upstream，通过 Cloudflare Workers TCP Sockets 连接域名或 IP 源站。</p><div class="actions"><a class="btn primary" href="https://${escapeHtml(runtime.adminHost)}">进入控制台</a><a class="btn secondary" href="https://gh.${escapeHtml(runtime.mainDomain)}">GitHub Route</a></div><div class="grid"><div class="item"><b>Virtual Host</b><span>按 Host 匹配子域名。</span></div><div class="item"><b>TCP Sockets</b><span>统一通过 TCP 发起 HTTP/HTTPS upstream 请求。</span></div><div class="item"><b>DNS + Host</b><span>支持域名解析、IP 源站和 Host Header 覆盖。</span></div></div></div></section></body></html>`;
}
...
function sanitizeRequestHeaders(request, route, bodyLength) {
...
function buildTcpHttpRequest(request, route, bodyBytes) {
  ...
  const headers = sanitizeRequestHeaders(request, route, bodyBytes.byteLength);
...
function addProxyDiagnosticHeaders(headers, route, extra = {}) {
  ...
  headers.set('X-Proxy-Transport', 'tcp');
